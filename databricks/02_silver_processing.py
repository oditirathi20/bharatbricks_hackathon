...
def build_silver_dataframe(bronze_df):
    ...
    engineered = (
        ...
        .withColumn(
            "income_bracket",
            F.when(F.col("income") < 100000, F.lit("EWS"))
            .when(F.col("income") < 300000, F.lit("LIG"))
            .when(F.col("income") < 1000000, F.lit("MIG"))
            .otherwise(F.lit("HIG")),
        )
        .withColumn(
            "land_category",
    ...
    tags_array = F.array(
        ...
        F.when(F.col("income") < 300000, F.lit("low_income")).otherwise(F.lit("")),
